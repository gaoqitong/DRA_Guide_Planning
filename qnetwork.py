...
class QNet(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, batch_size, save_path, device):
        self.sess = sess
        ...
        self.update_target_net_params = [self.target_net_params[i]
                                         .assign(tf.multiply(self.tau, self.net_params[i+1])
    ...
    def predict_q_from_save(self, states, restore_path):
        self.saver.restore(self.sess, restore_path)
        return self.sess.run(self.q_values, feed_dict={self.inputs: states})
